...
export function Main() {
  ...
  const alreadyLoadedOnMount = useRef(loadProgress === 1).current
  const [showingProgressBar, setShowingProgressBar] = useState(
    !alreadyLoadedOnMount,
  )
  const [showingSearchBar, setShowingSearchBar] = useState(alreadyLoadedOnMount)
  useEffect(() => {
    if (loadProgress === 1 && !alreadyLoadedOnMount) {
